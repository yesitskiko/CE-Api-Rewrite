...
util.is_prologue = function(address)
    if not (address % 16 == 0) then
        return false
    end

    return ((util.read_byte(address) == 0x55 and util.read_byte(address + 1) == 0x8B and
        util.read_byte(address + 2) == 0xEC) or -- push ebp | mov ebp,esp
        (util.read_byte(address) == 0x53 and util.read_byte(address + 1) == 0x8B and util.read_byte(address + 2) == 0xDC) or -- push ebx | mov ebx,esp
        (util.read_int32(address - 4) == 0xCCCCCCCC)) -- alignment?
...
util.scan_xrefs = function(str, nresult)
    ...
    if not result then
        error("Aob scan was not successful! Result: %s", result)
    end
...
util.create_detour = function(location, on_execute, event_trigger)
    ...
    local bytes = {
        ...
        0,
        0x74,
        0x08, -- je skip_jmp_back
        0x9D, -- popfd
        0x61, -- popad
        0xFF,
        0x25, -- jmp dword ptr [jmpback_location]
        bytes_jmpback[1],
        bytes_jmpback[2],
        bytes_jmpback[3],
        bytes_jmpback[4],
        ...
        0, -- push 1
        0xFF,
        0x15, -- call dword ptr [Kernel32.Sleep]
        bytes_sleep[1],
        bytes_sleep[2],
        bytes_sleep[3],
        bytes_sleep[4],
    ...
    while not detour_data.running do
        Sleep(1)
    end
...
util.new_detour = function(location, reg32, reg_offset, count, new_value)
    ...
    local bytes = {
        ...
        0x89,
        5 + (debug_reg * 8), -- mov [actual reg value], debug_reg
        bytes_reg_value[1],
        bytes_reg_value[2],
        bytes_reg_value[3],
        bytes_reg_value[4]
    ...
    if new_value ~= nil then
        ...
        if reg_offset == nil then
            ...
            local appendBytes = {
                0x8B,
                5 + (debug_reg * 8), -- mov debug_reg, [update_location]
                bytes_update[1],
                bytes_update[2],
                bytes_update[3],
                bytes_update[4]
            ...
            local appendBytes = {
                0x8B,
                5 + (r1 * 8), -- mov reg, [update_location]
                bytes_update[1],
                bytes_update[2],
                bytes_update[3],
                bytes_update[4],
                0x89,
                0x80 + debug_reg + (r1 * 8), -- mov [debug_reg + reg_offset], reg
                bytes_reg_offset[1],
                bytes_reg_offset[2],
                bytes_reg_offset[3],
                bytes_reg_offset[4]
    ...
    if count > 0 then
        tcombine(
            bytes,
            {
                ...
                0x44 + (r1 * 8),
                (r2 * 8) + debug_reg,
                0 --[[reg_offset]], -- mov reg,[debug_reg + reg + reg_offset]
                0x89,
                0x80 + (r1 * 8) + r2, -- mov [reg+OUTPUT_LOCATION],reg
                bytes_output[1],
                bytes_output[2],
                bytes_output[3],
                bytes_output[4],
...
util.new_remote = function(options)
    ...
    fremote.init = function()
        ...
        local bytes = {
            ...
            0x57, -- push edi
            0x8B,
            0x3D, -- mov edi, dword ptr [function_id_location]
            bytes_function_id[1],
            bytes_function_id[2],
            bytes_function_id[3],
            bytes_function_id[4],
            ...
            0xF2, -- je wait_async
            0xFF,
            0x25, -- jmp dword ptr [function_id_location]
            bytes_function_id[1],
            bytes_function_id[2],
            bytes_function_id[3],
            bytes_function_id[4],
